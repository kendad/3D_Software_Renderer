...
polygon_t create_polygon_from_triangle(triangle_t triangle) {
  polygon_t polygon = {.vertices = {triangle.vertices[0], triangle.vertices[1],
                                    triangle.vertices[2]},
                       .view_space_vertices = {triangle.view_space_vertices[0],
                                               triangle.view_space_vertices[1],
                                               triangle.view_space_vertices[2]},
...
void clip_polygon_against_axis(polygon_t *polygon, clipping_axis axis) {
  ...
  vec4_t inside_view_space_vertices[MAX_NUM_POLYGON_VERTICES];
  ...
  vec4_t *current_view_space_vertex = &polygon->view_space_vertices[0];
  ...
  vec4_t *previous_view_space_vertex =
      &polygon->view_space_vertices[polygon->num_vertices - 1];
  ...
  while (current_vertex != &polygon->vertices[polygon->num_vertices]) {
    ...
    if ((is_current_vertex_inside && !is_previous_vertex_inside) ||
      ...
      // interpolated view space coord for the intersection point
      vec4_t interpolated_view_space_vertex = {
          .x = lerp(previous_view_space_vertex->x, current_view_space_vertex->x,
                    t),
          .y = lerp(previous_view_space_vertex->y, current_view_space_vertex->y,
                    t),
          .z = lerp(previous_view_space_vertex->z, current_view_space_vertex->z,
                    t),
          .w = lerp(previous_view_space_vertex->w, current_view_space_vertex->w,
                    t),
      };

      // interpolated normal of the intersection point
      ...
      // interpolated texture coord of the intersection
      // point
      ...
      // put the new itersection point and its properties inside the list
      inside_vertices[num_inside_vertices] = intersection_point;
      inside_view_space_vertices[num_inside_vertices] =
          interpolated_view_space_vertex;
    ...
    if (is_current_vertex_inside) {
      ...
      inside_view_space_vertices[num_inside_vertices] =
          vec4_clone(current_view_space_vertex);
    ...
    previous_view_space_vertex = current_view_space_vertex;
    previous_normal = current_normal;
    previous_texcoord = current_texcoord;

    current_vertex++;
    current_view_space_vertex++;
    current_normal++;
    current_texcoord++;
  }
  ...
  for (int i = 0; i < num_inside_vertices; ++i) {
    ...
    polygon->view_space_vertices[i] = inside_view_space_vertices[i];
...
void triangle_from_polygon(polygon_t *polygon, triangle_t *triangles,
  ...
  for (int i = 0; i < polygon->num_vertices - 2; ++i) {
    ...
    triangles[i].view_space_vertices[0] = polygon->view_space_vertices[index0];
    triangles[i].view_space_vertices[1] = polygon->view_space_vertices[index1];
    triangles[i].view_space_vertices[2] = polygon->view_space_vertices[index2];
